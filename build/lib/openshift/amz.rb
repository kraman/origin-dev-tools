    ...
    def setup_rsa_key
      unless File.exists?(RSA)
        ...
        if not File.exist?(ssh_key)
          puts %{
No SSH key available to log into EC2 instance. 
Please create one and place it in #{RSA}
          }
          raise raise "Error - no credentials"
        else
    ...
    def terminate_ec2_instance(instance, handle_authdenied=false)
      begin
        (0..4).each do
          instance.terminate
          (0..12).each do
    ...
    def launch_ec2_instance(image, name, max_retries = 1)
